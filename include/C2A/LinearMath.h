...
public:
  ...
  inline Coord3D(Real x, Real y, Real z) { Set_Value(x, y, z); }
  ...
  // Point transformation
  friend Coord3D operator*(const Transform &, const Coord3D &);
  // Vector transformation
  friend Coord3D operator&(const Transform &, const Coord3D &);
...
public:
  ...
  inline Quaternion(Real x, Real y, Real z, Real w) { Set_Value(x, y, z, w); }
  inline Quaternion(Real t[]) { Set_Value(t); }
  // Creates a quaternion [x,y,z,0]
  inline Quaternion(const Coord3D &t) { Set_Value(t); }
  // Axis angle constructor
  inline Quaternion(const Coord3D &t, Real theta) { Set_Value(t, theta); }
  // Euler angle constructor (first rotate about x, then y, then z)
  inline Quaternion(Real x, Real y, Real z) { Set_Value(x, y, z); }
  inline ~Quaternion() {}
  ...
  inline Real Length() const {
    return Real(sqrt(val[0] * val[0] + val[1] * val[1] + val[2] * val[2] +
                     val[3] * val[3]));
  }
...
public:
  ...
  inline Matrix3x3(Real v0, Real v1, Real v2, Real v3, Real v4, Real v5,
  ...
  inline void Set_Value(Real rotx, Real roty, Real rotz) {
    Real cos_a = cos(rotx);
    Real sin_a = sin(rotx);
    Matrix3x3 rx(1.0, 0.0, 0.0, 0.0, cos_a, -sin_a, 0.0, sin_a, cos_a);
    cos_a = cos(roty);
    sin_a = sin(roty);
    Matrix3x3 ry(cos_a, 0.0, sin_a, 0.0, 1.0, 0.0, -sin_a, 0.0, cos_a);
    cos_a = cos(rotz);
    sin_a = sin(rotz);
    Matrix3x3 rz(cos_a, -sin_a, 0.0, sin_a, cos_a, 0.0, 0.0, 0.0, 1.0);
    *this = rz * (ry * rx);
  }
  ...
  Real Cofac(int r1, int c1, int r2, int c2) const {
    return val[r1 * 3 + c1] * val[r2 * 3 + c2] -
           val[r1 * 3 + c2] * val[r2 * 3 + c1];
  }
  //--------------zhangxy Feb 11, 2006
  ...
  friend Matrix3x3 operator*(Real s, const Matrix3x3 &m);
  friend Matrix3x3 operator*(const Matrix3x3 &m, Real s);
  friend Matrix3x3 operator/(const Matrix3x3 &m, Real s);

  friend Coord3D operator*(const Coord3D &t, const Matrix3x3 &m);
  friend Coord3D operator*(const Matrix3x3 &m, const Coord3D &t);
  // m^T * t
  friend Coord3D operator%(const Matrix3x3 &m, const Coord3D &t);
  ...
  friend void Coord3D::operator&=(const Transform &m);
  friend Coord3D operator*(const Transform &m, const Coord3D &t);
  friend Coord3D operator&(const Transform &m, const Coord3D &t);
...
public:
  ...
  inline Transform(const Matrix3x3 &matrix, const Coord3D &t)
      : R(matrix), T(t) {}
  inline Transform(const Transform &val) : R(val.R), T(val.T) {}
  // Set as a row major 3x4 matrix where the fourth column is the translation
  inline Transform(Real v0, Real v1, Real v2, Real v3, Real v4, Real v5,
                   Real v6, Real v7, Real v8, Real v9, Real v10, Real v11)
...
// Point transformation
inline Coord3D operator*(const Transform &m, const Coord3D &t);
// Vector transformation
inline Coord3D operator&(const Transform &m, const Coord3D &t);
...
inline void Coord3D::Get_Value(Real &x, Real &y, Real &z) const {
...
inline void Coord3D::Normalize() {
  if (this->Length() > 1.0e-20)
    *this /= this->Length();
}

inline void Coord3D::Zero() { val[0] = val[1] = val[2] = 0.0; }

inline void Coord3D::Identity() { Zero(); }
...
inline void Coord3D::operator^=(const Transform &m) {
  Real temp0 = val[0] * m.R.val[0] + val[1] * m.R.val[1] + val[2] * m.R.val[2] +
               m.T.val[0];
  Real temp1 = val[0] * m.R.val[3] + val[1] * m.R.val[4] + val[2] * m.R.val[5] +
               m.T.val[1];
...
// Axis angle setter
inline void Quaternion::Set_Value(const Coord3D &t, Real theta) {
  Coord3D unit_axis = t;
  unit_axis.Normalize();
...
inline void Quaternion::Set_Value(Real x, Real y, Real z) {
...
inline void Quaternion::Zero() { val[0] = val[1] = val[2] = val[3] = 0.0; }
...
inline Quaternion Matrix3x3::Quaternion_() {
  ...
  if (trace > 0.0) {
    ...
    int i =
        val[0] < val[4] ? (val[4] < val[8] ? 2 : 1) : (val[0] < val[8] ? 2 : 0);
    int j = (i + 1) % 3;
    int k = (i + 2) % 3;
...
inline void Transform::Set_Value(const Real r[], const Real t[]) {
...
// Dot product
inline Real operator*(const Coord3D &t1, const Coord3D &t2) {
  return Real(t1.val[0] * t2.val[0] + t1.val[1] * t2.val[1] +
              t1.val[2] * t2.val[2]);
}
...
inline Real operator*(const Quaternion &q1, const Quaternion &q2) {
  return q1.val[0] * q2.val[0] + q1.val[1] * q2.val[1] + q1.val[2] * q2.val[2] +
         q1.val[3] * q2.val[3];
}
...
// Matrix negation
inline Matrix3x3 operator-(const Matrix3x3 &m) {
  return Matrix3x3(-m.val[0], -m.val[1], -m.val[2], -m.val[3], -m.val[4],
                   -m.val[5], -m.val[6], -m.val[7], -m.val[8]);
}
...
// Scalar multiplication
inline Matrix3x3 operator*(Real s, const Matrix3x3 &m) {
  return Matrix3x3(s * m.val[0], s * m.val[1], s * m.val[2], s * m.val[3],
                   s * m.val[4], s * m.val[5], s * m.val[6], s * m.val[7],
                   s * m.val[8]);
}
...
inline Coord3D operator*(const Transform &m, const Coord3D &t) {
  return Coord3D(t.val[0] * m.R.val[0] + t.val[1] * m.R.val[1] +
                     t.val[2] * m.R.val[2] + m.T.val[0],
                 t.val[0] * m.R.val[3] + t.val[1] * m.R.val[4] +
                     t.val[2] * m.R.val[5] + m.T.val[1],
                 t.val[0] * m.R.val[6] + t.val[1] * m.R.val[7] +
                     t.val[2] * m.R.val[8] + m.T.val[2]);
}
