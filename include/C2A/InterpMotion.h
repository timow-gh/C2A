...
enum GMP_INTERP_MODE {
  ...
  GMP_IM_LINEAR = 1, // constant rotation velocity and constant translation
...
public:
  CScrewMotion(const PQP_REAL R0[3][3], const PQP_REAL T0[3],
               const PQP_REAL R1[3][3], const PQP_REAL T1[3]);
  CScrewMotion(const PQP_REAL q0[7], const PQP_REAL q1[7]);
  CScrewMotion();
};

class CInterpMotion {
public:
  CInterpMotion(GMP_INTERP_MODE itpMode, const PQP_REAL R0[3][3],
                const PQP_REAL T0[3], const PQP_REAL R1[3][3],
                const PQP_REAL T1[3]);

  CInterpMotion(GMP_INTERP_MODE itpMode, const PQP_REAL q0[7],
                const PQP_REAL q1[7]);
  ...
  virtual void velocity_screw(const PQP_REAL R0[3][3], const PQP_REAL T0[3],
                              const PQP_REAL R1[3][3], const PQP_REAL T1[3]);
  ...
  // compute the TOC using directional motion bound, //use the C2A_BV's angular
  // radius to compute TOC,
  virtual double computeTOC(PQP_REAL d, PQP_REAL r1, PQP_REAL S[3]);

  // compute the TOC using directional motion bound, //use the C2A_BV's angular
  // radius to compute TOC,
  virtual double computeTOC_MotionBound(PQP_REAL T[3], PQP_REAL d, C2A_BV *V,
                                        PQP_REAL N[3]);

  virtual bool CAonRSS(PQP_REAL r1[3][3], PQP_REAL tt1[3], PQP_REAL r2[3][3],
                       PQP_REAL tt2[3], PQP_REAL R[3][3], PQP_REAL T[3],
                       C2A_BV *b1, C2A_BV *b2, PQP_REAL *mint,
                       PQP_REAL *distance);

  virtual bool CAonNonAdjacentTriangles(PQP_REAL R[3][3], PQP_REAL T[3],
                                        PQP_REAL triA[3][3],
                                        PQP_REAL triB[3][3], PQP_REAL *mint,
                                        PQP_REAL *distance);
...
public:
  ...
  Transform
      m_transform_t_inv; // the inverse of transform_t: avoid the comptuation of
                         // the inverse during each integration
...
public:
  ...
  // use the angular radius computed from the nearest points to compute TOC,
  // more efficient than using C2A_BV's angular radius in computeTOC
  virtual double computeTOC(PQP_REAL d, PQP_REAL r1, PQP_REAL S[3]);

  // compute the TOC using directional motion bound, //use the C2A_BV's angular
  // radius to compute TOC,
  virtual double
  computeTOC_MotionBound(PQP_REAL T[3], PQP_REAL d, C2A_BV *V,
