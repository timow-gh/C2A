...
inline void Mat_V(PQP_REAL i[3], PQP_REAL j[3], PQP_REAL k[3],
...
CScrewMotion::CScrewMotion(const PQP_REAL R0[3][3], const PQP_REAL T0[3],
...
CScrewMotion::CScrewMotion(const PQP_REAL q0[7], const PQP_REAL q1[7]) {
  ...
  double sign = s < 0 ? -1 : 1;
  double a = (fabs(s - 1) <= 1e-40 || fabs(s + 1) <= 1e-40)
                 ? (2 * sign)
                 : (sign * acos(2 * s * s - 1) / sqrt(1 - s * s));
...
CInterpMotion::CInterpMotion(GMP_INTERP_MODE itpMode, const PQP_REAL R0[3][3],
                             const PQP_REAL T0[3], const PQP_REAL R1[3][3],
                             const PQP_REAL T1[3]) {
...
CInterpMotion::CInterpMotion(GMP_INTERP_MODE itpMode, const PQP_REAL q0[7],
                             const PQP_REAL q1[7]) {
  m_itpMode = itpMode;
  SetSrcDst(q0, q1);
  velocity();
...
void CInterpMotion::velocity_screw(const PQP_REAL R0[3][3],
                                   const PQP_REAL T0[3],
...
void CInterpMotion::LinearAngularVelocity(Coord3D &axis, PQP_REAL &angVel) {
  ...
  Quaternion s_q0(-quaternion_s.X(), -quaternion_s.Y(), -quaternion_s.Z(),
                  quaternion_s.W());

  Quaternion s_q1(quaternion_t.X(), quaternion_t.Y(), quaternion_t.Z(),
                  quaternion_t.W());
  ...
  s_q0.Set_Value(-quaternion_s.X(), -quaternion_s.Y(), -quaternion_s.Z(),
                 quaternion_s.W());
  s_q1 = quaternion_t;
...
bool CInterpMotion::integrate(const double dt_input, PQP_REAL R[3][3],
                              PQP_REAL T[3]) {
  PQP_REAL qua[7];
...
double CInterpMotion::computeTOC_MotionBound(PQP_REAL T[3], PQP_REAL d,
                                             C2A_BV *V, PQP_REAL N[3]) {
  PQP_REAL path_max = 0;
  return path_max;
}
...
bool CInterpMotion::CAonRSS(PQP_REAL r1[3][3], PQP_REAL tt1[3],
                            ...
                            PQP_REAL R[3][3], PQP_REAL T[3], C2A_BV *b1,
                            C2A_BV *b2, PQP_REAL *mint, PQP_REAL *distance) {
  bool flag = true;
  return flag;
}
...
double CInterpMotion_Linear::computeTOC(PQP_REAL d, PQP_REAL r1) {
  ...
  PQP_REAL w_max = r1 * fabs(m_angVel); // m_angVel;  the angular velocity
                                        // around that axis: counter clockwise
                                        //  it might be negative, which means
  ...
  time = ConservD(d) /
         path_max; // ConservD = d - security_distance_ratio * m_toc_delta
...
void CInterpMotion_Linear::velocity_screw(const PQP_REAL q0[7],
                                          const PQP_REAL q1[7]) {
...
void CInterpMotion_Linear::velocity_screw(const PQP_REAL R0[3][3],
                                          const PQP_REAL T0[3],
                                          const PQP_REAL R1[3][3],
                                          const PQP_REAL T1[3]) {
...
CInterpMotion_Linear::CInterpMotion_Linear(const PQP_REAL R0[3][3],
                                           const PQP_REAL T0[3],
                                           const PQP_REAL R1[3][3],
                                           const PQP_REAL T1[3])
    : CInterpMotion(GMP_IM_LINEAR, R0, T0, R1, T1) {
#ifdef SCREW_MOTION
...
CInterpMotion_Linear::CInterpMotion_Linear(const PQP_REAL q0[7],
                                           const PQP_REAL q1[7])
    : CInterpMotion(GMP_IM_LINEAR, q0, q1) {
#ifdef SCREW_MOTION
...
bool CInterpMotion_Linear::CAonNonAdjacentTriangles(
    PQP_REAL R[3][3], PQP_REAL T[3], PQP_REAL triA[3][3], PQP_REAL triB[3][3],
...
double CInterpMotion_Linear::computeTOC_MotionBound(PQP_REAL T[3], PQP_REAL d,
                                                    C2A_BV *V, PQP_REAL N[3]) {
