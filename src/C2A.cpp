...
int C2A_TimeOfContactStep(CInterpMotion *objmotion1, CInterpMotion *objmotion2,
                          C2A_TimeOfContactResult *res, PQP_REAL R1[3][3],
                          PQP_REAL T1[3], C2A_Model *o1, PQP_REAL R2[3][3],
                          PQP_REAL T2[3], C2A_Model *o2, PQP_REAL tolerance_t,
...
static void
ClosestPoints(PQP_REAL VEC[3], PQP_REAL X[3], PQP_REAL Y[3], // closest points
...
TriDist(PQP_REAL P[3], PQP_REAL Q[3], const PQP_REAL S[3][3],
        const PQP_REAL T[3][3], C2A_ContactFeature &f1, C2A_ContactFeature &f2,
        bool &bCollided) {
...
C2A_TriDistance(PQP_REAL R[3][3], PQP_REAL T[3], Tri *t1, Tri *t2,
                PQP_REAL p[3], PQP_REAL q[3], C2A_ContactFeature &f1,
                C2A_ContactFeature &f2, bool &bCollided) {
...
void ContactQueueRecurse(C2A_ContactResult *res, PQP_REAL R[3][3],
                         PQP_REAL T[3], C2A_Model *o1, int b1, C2A_Model *o2,
                         int b2) {
  ...
  while (1) {
    ...
    if (l1 && l2) {
      ...
      PQP_REAL d = C2A_TriDistance(res->R, res->T, t1, t2, p, q, f1, f2,
                                   res->modelcollided);
      ...
      ContactQueueRecurse(res, min_test.R, min_test.T, o1, min_test.b1, o2,
                          min_test.b2);
      if (res->modelcollided) {
        return;
      }
      ...
      if (l2 || (!l1 && (sz1 > sz2))) {
        ...
        bvt1.d = C2A_BV_Distance(bvt1.R, bvt1.T, (C2A_BV *)o1->child(bvt1.b1),
                                 (C2A_BV *)o2->child(bvt1.b2), S);
        ...
        bvt2.d = C2A_BV_Distance(bvt2.R, bvt2.T, (C2A_BV *)o1->child(bvt2.b1),
                                 (C2A_BV *)o2->child(bvt2.b2), S);
      } else {
...
//
static void DeleteDuplicatePairs(C2A_ContactResult *res, PQP_REAL R1[3][3],
                                 PQP_REAL T1[3], C2A_Model *o1,
  ...
  if (iPairsNum > maxPair) {
    // When the parameter for the contact query is very large, the number
    // iPairsNum could be huge: for instance 342000. At this time, just truncate
    // it.
    static int iWarning = 0;
    if (iWarning < 10) {
      iWarning++;
      printf("Warning %d: Wrong with the paramters of contact query. The "
             "number of paris is %d\n",
             iWarning, iPairsNum);
    }
        ...
        if ((res->cpairs[i].f1.type == res->cpairs[j].f1.type) &&
          ...
          if (res->cpairs[i].f1.type == 0) {
            ...
            if ((iTri == jTri) && (tri1[iVert][0] == tri2[jVert][0]) &&
                (tri1[iVert][1] == tri2[jVert][1] &&
                 (tri1[iVert][2] == tri2[jVert][2]))) {
              bDistinct = false;
              break;
            }
          }
          ...
          if (res->cpairs[i].f1.type == 1) {
            ...
            if ((((iVtri1[iEdge1][0] == jVtri1[jEdge1][0]) &&
                  (iVtri1[iEdge1][1] == jVtri1[jEdge1][1]) &&
                  (iVtri1[iEdge1][2] == jVtri1[jEdge1][2])) ||
                 ((iVtri1[(iEdge1 + 1) % 3][0] ==
                   jVtri1[(jEdge1 + 1) % 3][0]) &&
                  (iVtri1[(iEdge1 + 1) % 3][1] ==
                   jVtri1[(jEdge1 + 1) % 3][1]) &&
                  (iVtri1[(iEdge1 + 1) % 3][2] ==
                   jVtri1[(jEdge1 + 1) % 3][2]))) ||
                (((iVtri1[iEdge1][0] == jVtri1[(jEdge1 + 1) % 3][0]) &&
                  (iVtri1[iEdge1][1] == jVtri1[(jEdge1 + 1) % 3][1]) &&
                  (iVtri1[iEdge1][2] == jVtri1[(jEdge1 + 1) % 3][2])) ||
                 ((iVtri1[(iEdge1 + 1) % 3][0] == jVtri1[jEdge1][0]) &&
                  (iVtri1[(iEdge1 + 1) % 3][1] == jVtri1[jEdge1][1]) &&
                  (iVtri1[(iEdge1 + 1) % 3][2] == jVtri1[jEdge1][2]))))

            {
              // compare the edge on model 2
              if ((((iVtri2[iEdge2][0] == jVtri2[jEdge2][0]) &&
                    (iVtri2[iEdge2][1] == jVtri2[jEdge2][1]) &&
                    (iVtri2[iEdge2][2] == jVtri2[jEdge2][2])) ||
                   ((iVtri2[(iEdge2 + 1) % 3][0] ==
                     jVtri2[(jEdge2 + 1) % 3][0]) &&
                    (iVtri2[(iEdge2 + 1) % 3][1] ==
                     jVtri2[(jEdge2 + 1) % 3][1]) &&
                    (iVtri2[(iEdge2 + 1) % 3][2] ==
                     jVtri2[(jEdge2 + 1) % 3][2]))) ||
                  (((iVtri2[iEdge2][0] == jVtri2[(jEdge2 + 1) % 3][0]) &&
                    (iVtri2[iEdge2][1] == jVtri2[(jEdge2 + 1) % 3][1]) &&
                    (iVtri2[iEdge2][2] == jVtri2[(jEdge2 + 1) % 3][2])) ||
                   ((iVtri2[(iEdge2 + 1) % 3][0] == jVtri2[jEdge2][0]) &&
                    (iVtri2[(iEdge2 + 1) % 3][1] == jVtri2[jEdge2][1]) &&
                    (iVtri2[(iEdge2 + 1) % 3][2] == jVtri2[jEdge2][2])))) {
                bDistinct = false;
                break;
              }
            }
  ...
  if (bPQPContact_Represenative) {
    ...
    PQP_REAL square_epislon = res->ctolerance * res->ctolerance *
                              PQPContact_radius * PQPContact_radius;
    ...
    for (i = 1; i < iDistNum; i++) {
      ...
      for (int j = 0; j < repNum; j++) {
        ...
        if ((VdistV2(p1, p3) <= square_epislon) &&
            (VdistV2(p2, p4) <= square_epislon)) {
          //				pRepIndex[repNum++] = i;
          //				break;
          bSame = true;
...
void ContactRecurse(C2A_ContactResult *res, PQP_REAL R[3][3],
  ...
  PQP_REAL d1 = C2A_BV_Distance(R1, T1, (C2A_BV *)o1->child(a1),
                                (C2A_BV *)o2->child(a2), S);
  PQP_REAL d2 = C2A_BV_Distance(R2, T2, (C2A_BV *)o1->child(c1),
                                (C2A_BV *)o2->child(c2), S);
...
C2A_ContactResult::~C2A_ContactResult() { delete[] cpairs; }

void C2A_ContactResult::FreePairsList() {
  num_cpairs = num_cpairs_alloced = 0;
  delete[] cpairs;
  cpairs = 0;
}
// may increase OR reduce mem usage
void C2A_ContactResult::SizeTo(int n) {
...
void C2A_ContactResult::Add(const C2A_ContactFeature &f1,
                            const C2A_ContactFeature &f2, PQP_REAL distance,
                            PQP_REAL p[], PQP_REAL q[]) {
  // Check whether a feature pair is duplicated or not
  bool bDuplicated = false;
  if (bPQPContact_Represenative) {
    // The tolerance $\kappa_r$
    PQP_REAL square_epislon =
        ctolerance * ctolerance * PQPContact_radius * PQPContact_radius;
...
// toc
double TOCStepRecurse_Dis(PQP_REAL tolerance_t, CInterpMotion *objmotion1,
                          CInterpMotion *objmotion2,
                          C2A_TimeOfContactResult *res, double R[3][3],
                          double T[3], C2A_Model *o1, int b1, C2A_Model *o2,
                          int b2, double delta) {
  // delta =0.0001;
  int l1 = o1->child(b1)->Leaf();
  int l2 = o2->child(b2)->Leaf();

  PQP_REAL r1[3][3], r2[3][3], tt1[3], tt2[3], result1[3], result2[3], temp1[3],
      temp2[3], cwc[3];
  ...
  if (l1 && l2) {
    ...
    if (dTri <= res->distance) {
      ...
      PQP_REAL motionbound1 = objmotion1->computeTOC(
          dTri, ((C2A_BV *)(o1->child(b1)))->angularRadius, S1); //
      PQP_REAL motionbound2 = objmotion2->computeTOC(
          dTri, ((C2A_BV *)(o2->child(b2)))->angularRadius, S2);
  ...
  double d1 =
      C2A_BV_Distance(R1, T1, (C2A_BV *)o1->child(a1), (C2A_BV *)o2->child(a2),
                      a1_clo, a2_clo, bValid_A_clo, S1);
  if (d1 != 0.0) {
    MxV(temp1, ((C2A_BV *)o1->child(a1))->R_loc, S1);
    MxV(S1, r1, temp1);
    VxS(S2, S1, -1);

    motion_bound1 = objmotion1->computeTOC_MotionBound(
        T1, d1, ((C2A_BV *)o1->child(a1)), S1);
    motion_bound2 = objmotion2->computeTOC_MotionBound(
        T1, d1, ((C2A_BV *)o2->child(a2)), S2);
  ...
  double d2 =
      C2A_BV_Distance(R2, T2, (C2A_BV *)o1->child(c1), (C2A_BV *)o2->child(c2),
                      c1_clo, c2_clo, bValid_C_clo, S1);
  if (d2 != 0.0) {
    ...
    motion_bound1 = objmotion1->computeTOC_MotionBound(
        T2, d2, ((C2A_BV *)o1->child(c1)), S1);
    motion_bound2 = objmotion2->computeTOC_MotionBound(
        T2, d2, ((C2A_BV *)o2->child(c2)), S2);
  ...
  if (d2 < d1) {
    ...
    if (minta < res->UpboundTOC &&
        ((d2 < (res->distance - res->abs_err)) ||
         (d2 * (1 + res->rel_err) < res->distance))) {
      TOCStepRecurse_Dis(tolerance_t, objmotion1, objmotion2, res, R2, T2, o1,
                         c1, o2, c2, delta);
    ...
    if (mintb < res->UpboundTOC &&
        ((d1 < (res->distance - res->abs_err)) ||
         (d1 * (1 + res->rel_err) < res->distance))) {
      TOCStepRecurse_Dis(tolerance_t, objmotion1, objmotion2, res, R1, T1, o1,
                         a1, o2, a2, delta);
    } else {
...
double TOCStepRecurse_Dis_Translation(
    PQP_REAL tolerance_t, CInterpMotion *objmotion1, CInterpMotion *objmotion2,
    C2A_TimeOfContactResult *res, double R[3][3], double T[3], C2A_Model *o1,
    int b1, C2A_Model *o2, int b2, double delta) {
  ...
  if (l1 && l2) {
    ...
    bool CAtri_flag = objmotion1->CAonNonAdjacentTriangles(
        r1, tt1, tri1, tri2, &res->mint, &res->distance);
  ...
  bool CARSS_flag_a =
      objmotion1->CAonRSS(r1, tt1, r2, tt2, R1, T1, (C2A_BV *)o1->child(a1),
                          (C2A_BV *)o2->child(a2), &minta, &d1);

  bool CARSS_flag_c =
      objmotion1->CAonRSS(r1, tt1, r2, tt2, R2, T2, (C2A_BV *)o1->child(c1),
                          (C2A_BV *)o2->child(c2), &mintc, &d2);
  ...
  if (d2 < d1) {
    ...
    if (mintc < res->mint && ((d2 < (res->distance - res->abs_err)) ||
                              (d2 * (1 + res->rel_err) < res->distance))) {
      TOCStepRecurse_Dis_Translation(tolerance_t, objmotion1, objmotion2, res,
                                     R2, T2, o1, c1, o2, c2, delta);
    }

    if (minta < res->mint && ((d1 < (res->distance - res->abs_err)) ||
                              (d1 * (1 + res->rel_err) < res->distance))) {
      TOCStepRecurse_Dis_Translation(tolerance_t, objmotion1, objmotion2, res,
                                     R1, T1, o1, a1, o2, a2, delta);
    }
...
void TOCStepRecurse_Dis_contact(C2A_TimeOfContactResult *res, PQP_REAL R[3][3],
                                PQP_REAL T[3], // b2 relative to b1
                                C2A_Model *o1, int b1, C2A_Model *o2, int b2,
                                int depth) {
  ...
  // depth, res->num_contact, res->cont_l.size()); res->num_contact = 0; return;
  //}
  ...
  if (l1 && l2) {
    ...
    C2A_Tri *t1 = o1->GetTriangle(trnum1); //(C2A_Tri*)&((C2A_Tri*)o1->tris)[-o1->child(b1)->first_child
                                           //- 1];
    C2A_Tri *t2 = o2->GetTriangle(trnum2); //(C2A_Tri*)&((C2A_Tri*)o2->tris)[-o2->child(b2)->first_child
                                           //- 1];
    C2A_ContactFeature f1, f2;
    bool bCollided = false;
    PQP_REAL d =
        C2A_TriDistance(res->R, res->T, t1, t2, p, q, f1, f2, bCollided);

    if (f1.type == -1 || f2.type == -1)
      return;
    if (d <= res->distance) {
      ...
      res->cont_l.push_front(ContactF(FeatureType_A, FeatureType_B, FeatureID_A,
                                      FeatureID_B, TriangleID_A, TriangleID_B,
                                      P_A, P_B, Distance));
...
int C2A_TimeOfContactStep_Contact(CInterpMotion *objmotion1,
                                  CInterpMotion *objmotion2,
                                  C2A_TimeOfContactResult *res,
                                  PQP_REAL R1[3][3], PQP_REAL T1[3],
                                  C2A_Model *o1, PQP_REAL R2[3][3],
                                  PQP_REAL T2[3], C2A_Model *o2,
                                  double threshold) {
...
int C2A_TimeOfContactStep(CInterpMotion *objmotion1, CInterpMotion *objmotion2,
  ...
  if (b_TanslationCCD) {
    ...
    bool CAtri_flag =
        objmotion1->CAonNonAdjacentTriangles(r1, tt1, tri1, tri2, &mint, &dTri);
    if (CAtri_flag) {
      res->mint = mint;
      res->distance = dTri;
    } else {
  ...
  if (b_TanslationCCD) {
    ...
    TOCStepRecurse_Dis_Translation(tolerance_t, objmotion1, objmotion2, res, R,
                                   T, o1, 0, o2, 0, tolerance_d);
    t1 = res->last_triA;
    t2 = res->last_triB;
    ...
    TOCStepRecurse_Dis(tolerance_t, objmotion1, objmotion2, res, R, T, o1, 0,
                       o2, 0, tolerance_d);
  }
...
PQP_REAL
C2A_QueryContact(CInterpMotion *objmotion1, CInterpMotion *objmotion2,
                 C2A_TimeOfContactResult *res, C2A_Model *o1, C2A_Model *o2,
                 double threshold) {
  PQP_REAL toc = 0;
  ...
  C2A_TimeOfContactStep_Contact(objmotion1, objmotion2, res, R1, T1, o1, R2, T2,
                                o2, threshold);
...
PQP_REAL
C2A_QueryContactOnly(C2A_TimeOfContactResult *res, PQP_REAL R1[3][3],
                     PQP_REAL T1[3], C2A_Model *o1, PQP_REAL R2[3][3],
                     PQP_REAL T2[3], C2A_Model *o2, double threshold) {
  res->cont_l.clear();
  res->num_contact = 0;
  res->UpboundTOC = 1;
...
// calculate the toc
PQP_REAL
C2A_QueryTimeOfContact(CInterpMotion *objmotion1, CInterpMotion *objmotion2,
                       C2A_TimeOfContactResult *res, C2A_Model *o1,
                       C2A_Model *o2, PQP_REAL tolerance_d,
                       PQP_REAL tolerance_t, int qsize) {
  ...
  C2A_TimeOfContactStep(objmotion1, objmotion2, res, R1, T1, o1, R2, T2, o2,
                        tolerance_t, tolerance_d);
  ...
  objmotion1->integrate(toc, res->R_toc,
                        res->T_toc); // make res->R_toc equals R1
...
C2A_Result C2A_Solve(Transform *trans00, Transform *trans01,
                     C2A_Model *obj1_tested, Transform *trans10,
                     Transform *trans11, C2A_Model *obj2_tested,
                     Transform &trans0, Transform &trans1,
                     PQP_REAL &time_of_contact, int &number_of_iteration,
                     int &number_of_contact, PQP_REAL th_ca,
  ...
  C2A_QueryTimeOfContact(&motion1, &motion2, &dres, obj1_tested, obj2_tested,
                         d_delta, t_delta, 0);
  ...
  if (!dres.collisionfree) {
    ...
    C2A_QueryContact(&motion1, &motion2, &dres, obj1_tested, obj2_tested,
                     threshold);
  }
