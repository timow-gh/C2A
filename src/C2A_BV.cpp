...
// Calculate the maximum length between the vertex of triangles which belong the
// C2A_BV
void C2A_BV::ComputeAngularRadius(PQP_REAL O[3][3], C2A_Tri *tris,
                                  int num_tris) {
  int i;
  angularRadius = 0.0;
  PQP_REAL d;
...
void C2A_BV::FitToTris(PQP_REAL O[3][3], C2A_Tri *tris, int num_tris,
                       PQP_REAL comR[3]) {

  VcV(comRoot, comR);                    // comRoot=comR
  ComputeCenterOfMassBV(tris, num_tris); // calculate the mass center of C2A_BV
  ComputeAngularRadius(O, tris,
                       num_tris); // calculate the maximum distance of points in
                                  // C2A_BV to the model center
...
void C2A_BV::FitToTris_Corner(PQP_REAL O[3][3], C2A_Tri *tris, int num_tris,
                              PQP_REAL comR[3]) {
  ...
  Length_C1 = Vlength(Corner[0]) < Vlength(Corner[2]) ? Vlength(Corner[0])
                                                      : Vlength(Corner[2]);

  MxV(Corner[1], R, Corner_temp[1]);
  MxV(Corner[3], R, Corner_temp[3]);
  Length_C2 = Vlength(Corner[1]) < Vlength(Corner[3]) ? Vlength(Corner[1])
                                                      : Vlength(Corner[3]);
...
int C2A_BV_Overlap(PQP_REAL R[3][3], PQP_REAL T[3], C2A_BV *b1, C2A_BV *b2) {
...
PQP_REAL
C2A_BV_Distance(PQP_REAL R[3][3], PQP_REAL T[3], C2A_BV *b1, C2A_BV *b2,
                PQP_REAL P[3], PQP_REAL Q[3], bool &bValidPQ, PQP_REAL S[3]) {
